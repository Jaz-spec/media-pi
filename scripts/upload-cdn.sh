...
# If record.sh is still writing this file, refuse. With the segment muxer the
# "in-flight file" is the highest-numbered part_NNN.mp4 in the active session
# dir; earlier parts are sealed and safe to upload.
if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
  ...
  if [[ -n "$current_session" && "$(dirname "$FILE")" == "$current_session" ]]; then
    ...
    if [[ "$latest" == "$FILE" ]]; then
      echo "upload-cdn.sh: $FILE is the in-flight segment — wait for next chunk to roll" >&2
      exit 2
    fi
  fi
fi
